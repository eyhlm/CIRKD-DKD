...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_dkd = DKD(temperature=args.kd_temperature).to(self.device)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            if not program_started_logged:
                ...
                logging.info(f"s_outputs shape:\n{s_outputs[0].shape}")
                # logging.info(f"部分:\n{targets[1,:,:]}")
            # kd_loss = self.args.lambda_kd * self.criterion_kd(s_outputs[0], t_outputs[0])
            dkd_loss = self.args.lambda_kd * self.criterion_dkd(s_outputs[0], t_outputs[0],targets)
            # print(dkd_loss)
            # print(self.criterion_kd(s_outputs[0], t_outputs[0]))
            logger.info("dkd_loss:{:.4f}".format(dkd_loss))
            ...
            losses = task_loss + dkd_loss + minibatch_pixel_contrast_loss +\
                memory_pixel_contrast_loss + memory_region_contrast_loss
            ...
            dkd_losses_reduced = self.reduce_mean_tensor(dkd_loss)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    "Iters: {:d}/{:d} || Lr: {:.6f} || Task Loss: {:.4f} || DKD Loss: {:.4f}" \
